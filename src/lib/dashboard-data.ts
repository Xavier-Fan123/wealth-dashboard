...
import { getQuotes, getFxRates, convertToSGD, type FxRates } from "@/lib/market";
...
export function normalizeTicker(asset: string): string {
  const normalized = asset.trim().toUpperCase();
  if (normalized === "GOLD ETF") return "GLD";
  return normalized;
}

export function parseTransferAsset(asset: string): {
  fromAccount: string;
  toEntity: string;
  toAccount: string;
} | null {
...
/**
 * Aggregated dashboard snapshot. This is the single source of truth for both the
 * `/api/dashboard` route and the monthly email report — keep the returned shape stable.
 *
 * The function additionally exposes `fxRates` (already part of the payload) so callers
 * such as the monthly report can convert raw transaction amounts to SGD consistently.
 */
export async function getDashboardData() {
  const [holdings, manualAssets, allTransactions, recentTransactions, balanceItems, fxRates] =
    await Promise.all([
  ...
  const familyAssetValue =
    familyEquityValue + familyCommodityValue + familyCashValue + familyReceivableValue;
  ...
  const allocation = [
    ...
    {
      name: "Receivables",
      value: roundValue(familyReceivableValue + companyReceivableValue),
      color: "#22c55e",
    },
...
export type DashboardSnapshot = Awaited<ReturnType<typeof getDashboardData>>;
export type { FxRates };
